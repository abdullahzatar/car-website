...
if(isset($_POST['accept'])){
  ...
  $sql = "UPDATE user_cars SET state='accept' WHERE id='".$_POST["id"]."'";
  ...
  if(isset($_POST['reject'])){
    ...
    $sql = "UPDATE user_cars SET state='reject' WHERE id='".$_POST["id"]."'";
    mysqli_query($conn, $sql);
...
<header class="header">
    ...
    <div>
       <p style="color:#FFFFFF  ; font-size:large; margin:10px ; text-align: end;"><?php echo $_SESSION['admin_name'] ?> 
...
$sql = "SELECT id,email,car_type,car_modle,date,state FROM user_cars";
...
if ($result->num_rows > 0) {
  echo "<table  style='margin: 50px; width: 1000px;font-size :15px; '><tr><th>ID</th><th>Email</th><th>Car Type</th><th>Car Modle</th><th>date</th><th>state</th><th>Edit</th></tr>";
  ...
  while($row = $result->fetch_assoc()) {
    ...
    <form action='' method='post'>
        ...
        <input type='submit' class='btn' name='accept' title='go' value='A' style='margin-top: 10px;' >
    </form>
    <form action='' method='post'>
        <input type='hidden' class='btn'name='id' value='".$row["id"]."'>
        <input type='submit' class='btn' name='reject' title='go' value='R' style='margin-top: 10px;'>
    </form>
    </div>
    </td>
